Attribute VB_Name = "GibbsSpikeRgpox"
' RGPOX Gibbs 迁移 Spike — 方案 B：n = n_p + B*z（Case-1 全链元素进料 @1400°C）
' 工作簿须含 RGX_* 表（由 build_gibbs_spike_workbook.py 生成）

Option Explicit

Private Const SHEET_SETUP As String = "RGX_Setup"
Private Const SHEET_COMPARE As String = "RGX_Compare"
Private Const SHEET_NP As String = "RGX_n_particular"
Private Const SHEET_NULL As String = "RGX_Null_B"
Private Const SHEET_MU0 As String = "RGX_mu0"
Private Const SHEET_Z As String = "RGX_z_solution"

Private Const N_SPEC As Long = 8
Private Const N_Z As Long = 3
Private Const PENALTY_OBJ As Double = 1E+20

Public Sub RunGibbsSpikeRgpoxCase1()
    On Error GoTo Fail
    ...
    MsgBox "RGPOX Gibbs Spike VBA 完成: " & msg, vbInformation
    Exit Sub
Fail:
    MsgBox "GibbsSpikeRgpox 错误 " & Err.Number & ": " & Err.Description, vbCritical
End Sub
...
Private Sub ReadSetup(ByRef T_K As Double, ByRef P_bar As Double, ByRef Rgas As Double, ByRef P_ratio As Double)
    ...
    If T_K < 200# Or P_bar <= 0# Or Rgas <= 0# Or P_ratio <= 0# Then
        Err.Raise vbObjectError + 1, "GibbsSpikeRgpox", "RGX_Setup 参数无效，请重新生成工作簿"
    End If
End Sub
